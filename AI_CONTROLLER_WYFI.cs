...
public class AI_CONTROLLER_WYFI : MonoBehaviour {
    ...
    public TankFase tankFase;
    public SearchingFase searchingFase;
    public AttackingFase attackingFase;
    ...
    void Update()
    {
        ...
        else
        {
            if (enemyDetected)
            {
                tankFase = TankFase.Attacking;
            } else if (!evadingWall)
            {
                tankFase = TankFase.Searching;
            }

            thinkingTimer = thinkingDelay;
        }

        if (tankFase == TankFase.Searching)
        {
            ...
            else
            {
                ...
                if (searchingFase == SearchingFase.Moving)
                {
                    searchingFase = SearchingFase.Rotating;
                }
                else
                {
                    searchingFase = SearchingFase.Moving;
                }
            }
            ...
            else if (searchingFase == SearchingFase.Rotating)
            {
                tankController.Turn(-1);
                if (enemyDetected)
                {
        ...
        else if(tankFase == TankFase.Attacking)
        {
            if (shooting)
            {
                ...
                else
                {
                    ...
                    if (attackingFase == AttackingFase.StraightShoot)
                    {
                        attackingFase = AttackingFase.ChargeShoot;
                    }
                    else
                    {
                        attackingFase = AttackingFase.StraightShoot;
                    }
            ...
            else if (attackingFase == AttackingFase.ChargeShoot)
            {
                if (Vector3.Distance(transform.position, tankController._infEnemyLastPos) > 3)
                {
                    tankController.Move(1);
                }
                else
...
public enum TankFase
{
    Searching,
    Attacking,
}

public enum SearchingFase
{
    Moving,
    Rotating
}

public enum AttackingFase
{
    StraightShoot,
    ChargeShoot
}